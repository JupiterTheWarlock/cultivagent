...
- 初速度由本发最终 seed 决定，禁止使用批次开始时的旧行星位置。

批次规则：

- 每发使用 `batch seed + shot index` 独立选一个最终 seed，禁止共享批次目标。
- seed 位于发射行星黄道投影的 180° 对面。
- 大批次可用加权视觉粒子，但逻辑 cloud 总数必须守恒。

单段飞行：

- 粒子从行星球心沿唯一一段 `v0 + t² × 恒星重力` 仿抛物线直接落入 seed。
- 禁止 maneuver、第二段推进、贝塞尔、圆弧和 Hermite。
- 弹道必须精确跨越恒星 180° 命中 seed，不穿过 seed 半径内侧。
- `v0` 与恒星重力项由 seed 及顺公转终速反解；终点切线严格沿 seed 公转切线。

入云表现：

- 发射粒子是单颗粒子，带短拖尾。
- 拖尾不是持续绘制的长轨迹线。
- 飞行中不淡出、无机动红闪；到达时产生明显的白色扩散大闪。
...
- 每发独立选点，多发不叠成同一条线。
- 粒子从行星球心出发，穿出球面后可见。
- 单段仿抛物线直接命中行星 180° 对面的最终 seed，不穿过 seed 半径内侧。
- 终速顺公转；不存在 maneuver 或第二段。
- 粒子入轨方向与云环公转方向一致。
